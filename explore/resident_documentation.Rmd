...
data_gen %>%
    filter(rec_month == cur_month | rec_month == cur_month - months(1)) %>%
    count(resident, rec_month) %>%
    mutate_at("resident", str_to_title) %>%
    mutate(current = if_else(rec_month == cur_month, "current", "previous")) %>%
    select(-rec_month) %>%
    spread(current, n, fill = 0) %>%
    arrange(desc(current), resident) %>%
    mutate_at("resident", as_factor) %>%
    ggplot(aes(x = resident)) +
    geom_bar(aes(y = current), stat = "identity", width = 0.5, fill = "grey65") +
